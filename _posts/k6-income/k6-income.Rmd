...
dl <- df %>%
 ...
 dplyr::filter(Wave != 2009) %>% # no k6 measure
  ...
  #                ))%>%
   dplyr::ungroup(Id) %>%
   dplyr::select(id, k6, k6_lag, inc_l_lag, age_lag, eth_lag,urban_lag,edu_lag,
                 church_l_lag,nzdep_lag,partner_lag,emp_lag,time,male_lag,mxtime, inc_l, Id) %>%
  drop_na() %>%
  dplyr::mutate(age_c_lag = scale(age_lag, scale = F)) %>%
...
## Prepare data for IP Weights (round 2)
...
dl_weights_2 <- ipw::ipwtm(
  ...
  id = "id",
  timevar = "time", 
  numerator = ~  inc_l_lag  +  # lag treatment + confounders
    age_c_lag +
    male_lag + 
    eth_lag + 
    emp_lag + 
...
dl_weights_use_2 <- dl %>% 
  mutate(ipw = dl_weights_2$ipw.weights)
```

Check weights -- no extreme weights 

```{r}
summary(dl_weights_use_2$ipw)
```
...
Without weights, we recover the effect. 
