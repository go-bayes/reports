...
Estimated marginal means: strong boost to Trust in Science during lockdown, which was dropped somewhat after lockdown. 
...
ggplot(cv19w, aes(x = Covid_Timeline, y = SCIENCE.TRUST)) +
  # Add base data
  geom_violin(aes(fill = Covid_Timeline, alpha=.02), color = "white") +
...
Trust in science increased, and then dropped. The key question, will it trust revert to baseline (or worse)?  Below another graph of the same result.
```{r}
