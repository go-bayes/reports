...
km_all <- df %>%
 ...
 # dplyr::filter(Attack == 0) %>%
  ...
  dplyr::mutate(wave = as.numeric())
  select(Id, Wave, Ys, As, Warm.Muslims, Attack, Age, EthnicCats, Edu, Employed, Urban, Male, GenCohort, Religious, Pol.Orient, Parent, Partner, Urban, NZdep) %>%
  dplyr::mutate(pol_bz = if_else(Wave == "2018", scale(Pol.Orient), NA_real_)) %>%
  fill(pol_bz) %>%
  dplyr::mutate(rel_bz = if_else(Wave == "2018", scale(Religious), NA_real_)) %>%
  fill(rel_bz) %>%
  dplyr::mutate(partner_bz = if_else(Wave == "2018", scale(as.numeric(Partner)), NA_real_)) %>%
  fill(partner_bz) %>%
  dplyr::mutate(parent_bz = if_else(Wave == "2018", scale(as.numeric(Parent)), NA_real_)) %>%
  fill(parent_bz) %>%
  dplyr::mutate(age_bz = if_else(Wave == "2018", scale(Age), NA_real_)) %>%
  fill(age_bz) %>%
    dplyr::mutate(nzdep_bz = if_else(Wave == "2018", scale(NZdep), NA_real_)) %>%
  fill(nzdep_bz) %>%
  dplyr::mutate(male_2z = if_else(Wave == "2018", scale(as.numeric(Male))/2, NA_real_)) %>%
  fill(male_2z) %>%
  dplyr::mutate(employed_bz = if_else(Wave == "2018", scale(as.numeric(Employed)), NA_real_)) %>%
  fill(employed_bz) %>%
  dplyr::mutate(edu_bz = if_else(Wave == "2018", scale(Edu), NA_real_)) %>%
  fill(edu_bz) %>%
  dplyr::mutate(ubran_bz = if_else(Wave == "2018", scale(as.numeric(Urban)), NA_real_)) %>%
  fill(ubran_bz) %>%
  dplyr::mutate(EthnicCats_b = if_else(Wave == "2018", EthnicCats, NA_real_)) %>%
  fill(EthnicCats_b) %>% 
...
# avoid collineraity
ka2<-ka%>%
  select(Id,Wave,wave,As,Ys,Edu,EthnicCats,Employed,Urban,Male,GenCohort,Religious,Pol.Orient,NZdep)
