...
km_all <- df %>%
 ...
 # dplyr::filter(Attack == 0) %>%
  ...
  dplyr::mutate(wave = as.numeric(Wave)-1) %>% select(Id, Wave, Ys, As, Warm.Muslims, Attack, Age, EthnicCats, Edu, Employed, Urban, Male, GenCohort, Religious, Pol.Orient, Parent, Partner, Urban, NZdep) %>%
  dplyr::mutate(pol_bz = if_else(Wave == "2018", scale(Pol.Orient), NA_real_)) %>%
  fill(pol_bz) %>%
  dplyr::mutate(rel_bz = if_else(Wave == "2018", scale(as.numeric(Religious)), NA_real_)) %>%
  fill(rel_bz) %>%
