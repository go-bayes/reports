...
ned17 <-
  wm_mus + wm_immigrants + wm_e + plot_annotation(title = "Histogram of warmth responses to different groups", subtitle = "`Please rate your feelings of WARMTH toward the following groups using the `feeling thermometer scale` for \neach group`source = NZAVS wave 9 (2017-2018), N = 16473") 
ned17
```
...
ned19<- (wm_mus2 + wm_immigrants2 + wm_e2) 

ned19
ned17 / ned19 + plot_annotation(title="Histogram of warmth responses to different groups pre/post shootings", subtitle ="`Please rate your feelings of WARMTH toward the following groups using the `feeling thermometer scale` for each group`\n source = NZAVS wave 9 & wave 11, n's = 16473/35339")
```
...
##   Model? | Warmth to minority groups is slowly increasing {.centered}
...
lds_0 + li_0 +  plot_layout(guides = 'collect', nrow = 1, byrow = FALSE) +   plot_annotation(title = "GAM of attitudes/years to (a) Muslims and (b) Migrants in New Zealand;  2011-2020: N = 22,254", subtitle = "Green line = maximum level of warmth for Muslims, red line = Mosque attacks", tag_levels = 'a')
...
## Results: Muslim-warmth boost  {.centered}
...
- There was shallower post-attack boost in warmth for non-mormative minorities.
- The attacks did not affect warmth towards normative minorities.
- It is unclear whether the boost to warm towards Muslims will be sustained.
</div>
