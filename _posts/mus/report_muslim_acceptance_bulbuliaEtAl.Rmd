...
grand_mean_ame_0 <- grand_mean_ame%>%
...
m3ac <- brms::brm_multiple(
 ...
 # backend = "cmdstanr",
  data = amelia.list,
  file = here::here("_posts", "mus", "mods", "m3ac.rds"))
summary(m3ac)
...
m3aa <- brm_multiple(Ys ~ As +  Wave + (1|Id), 
  data = list(imputed$imputations$imp1,imputed$imputations$imp2), 
 # seed = 1234,
  warmup = 1000,
  iter = 2000,
  chains = 4,
  backend = "cmdstanr",
  file = here::here("_posts", "mus", "mods", "m3aa.rds"))


library(lazerhawk)
brms_SummaryTable(m3)


pred <- ggeffects::ggemmeans(m3, terms = c("As","Wave"))
