...
## Appendix 4: Predictors of Muslim acceptance

We observe that political conservativism is strongly associated with minority gropu prejudice. In absolute terms, however, the growth in Muslim acceptance following the shootings was stronger at the conservative end of the scale (see Figure \@ref(fig:graphsmodelsPol). The graph plots all expected values, as such the darkness of the areas reflect densities of responses. This approach allows us to visualise the variance around the average responses (the))

```{r  graphsmodelsPol, fig.fullwidth = TRUE,  layout="l-body-outset", fig.width=12, fig.height=10, cache = TRUE, fig.cap="\\label{fig:graphsmodelsPol} Warmth to Muslims (y-axis) and other groups by political conservativism (x-axis) pre/post shootings (the solid lines). The shift betweenand the purple/blue lines (pre-attacks) and the yellow/green lines (post-attacks) show magnitude of growth in minority group acceptance. Results reveal that the attacks led to a greater increase in acceptance of Muslims among political conservatives. However, this population had more headroom for growth in warmth. Whether this boost in Muslim acceptance endures remains to be seen. (N = 11272, Years = 2017-2020, Source = New Zealand Attitudes and Values Study)", eval=TRUE, echo = FALSE}
