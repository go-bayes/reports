...
The New Zealand Attitudes and Values Study (NZAVS) is a nationally representative panel study started in 2009 [@nzavs2021], which has measured over 65,000 New Zealanders (see [NZAVS](https://go-bayes.github.io/reports/posts/nzavs/))
...
```{r pptimeline, cache=TRUE, fig.fullwidth = TRUE, fig.width=12, fig.height=8, fig.cap="\\label{fig:pptimeline} Within-participant repeated measures covering the years before and following the attacks allow precise estimates of attitudinal change within-people (N = 11,481)."}
## timeline
rarep<-df%>%
  dplyr::filter(YearMeasured == 1) %>%
...
```{r warmthtime, fig.width=10, fig.height=24, fig.cap="\\label{fig:warmthtime} Within-person prejudice change pre/post attacks towards (a) Muslims, (b) Immigrants, (c) Māori, (d) NZ Europeans. Redline indicates mosque attacks. Greenline indicates maximum warmth to Muslims, which was strongly boosted post-attacks, yet lags behind other groups", cache = TRUE, fig.fullwidth = TRUE, include = TRUE}
