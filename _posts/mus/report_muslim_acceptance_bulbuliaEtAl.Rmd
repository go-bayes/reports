...
x <- table1::table1(~Age + Male +Edu + EthnicCats  +  Religious + Pol.Orient + Urban + NZdep|Wave*Attack, data = km0, overall=FALSE)
...
p0 <- ggeffects::ggemmeans(m00,
                           terms = c("Wave[2018,2019]","Attack[0]"))
...
d0 <-m1 %>% 
  emmeans(~ Wave,
          ...
          epred = TRUE,
          re_formulat = NA )
d0<- saveRDS(d0,  here::here("_posts", "mus", "mods", "d0.rds"))
...
grand_mean_ame
plot_all__ame <- ggplot(grand_mean_ame,
                                     aes(x = .value)) +
...
# dplyr::mutate(Pre_PostATTACK = forcats::fct_relevel(Pre_PostATTACK, ord_attack)) %>%
  mutate(wm_lead = lead(Warm.Muslims)) %>%
  mutate(years = (TSCORE - min(TSCORE)/365)) 

 # filter(Wave == 2018) %>%
#  drop_na()

length(km2$Id) # 34446
str(km2$Pre_PostATTACK)
...
#  
#    # dplyr::mutate(
#   #   years = (TSCORE - min(TSCORE) / 365) %>%
#   #     dplyr::mutate(Attack = ifelse(TSCORE >= 3545, 1, 0)) %>%
#   #     dplyr::mutate(
#   #       daysAfter = as.integer(TSCORE - 3545),
#   #       yearsAfter = daysAfter / 365
#   #     ) %>%
...
#   #     filter(n == 3) %>%
#   #     dplyr::ungroup() %>%
#       # mutate(value_tmp = if_else(Wave == "2017", Warm.Muslims, NA_real_))%>%
#       #   fill(value_tmp) %>%
#       #   mutate(bs_mw = value_tmp) %>%
#     
# m00 <- lmer(wm_lead ~   Wave + Attack +   (1|Id), 
#            data = km2)
# 
# parameters::model_parameters(m00)%>%
#   print_html()
# p0 <- ggeffects::ggemmeans(m00,
#                            terms = c("Wave[2019]","Attack[0,1]"))
# p0
# 
# 
# # for graph 
