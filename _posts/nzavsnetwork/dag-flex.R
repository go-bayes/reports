...
A key challenge in longitudional research is participant retention. This graph presents sample retention for the New Zealand Attitudes and Values Study (NZAVS). Figure \@ref(fig:graphalluvial) presents the sample frames within a single graph. Figure \@ref(fig:graphfacet) presents the sample frames separately. 
...
```{r graphalluvial, fig.cap = "NZAVS alluvial retention graph: waves 2009-2020."}
...
```{r graphfacet, fig.cap="NZAVS alluvial retention graph: facets are sample frames."}
# prepare data
datsSS <- ssdf.01 %>%
...
## Multilevel model of state change (not to publish just yet)
