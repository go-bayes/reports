...
c1 <- correlation::correlation(df, select = "Relid", select2 = "Spiritual.Identification") 
...
```{r}
#model 
m1 <- glm(Issue.Euthanasia ~  scale(Spiritual.Identification) * scale(Relid) + 
            scale(Age), data = df)
```
...
```{r}
library(viridisLite)
plot(ggeffects::ggpredict(m1, terms = c( "Spiritual.Identification")), add.data = TRUE, dot.alpha = 0.1) + scale_y_continuous(limits = c(1,7)) + scale_colour_viridis_c() + 
...
```{r}
library(viridisLite)
plot(ggeffects::ggpredict(m1, terms = c( "Spiritual.Identification", "Relid")), add.data = TRUE, dot.alpha = 0.1) + scale_y_continuous(limits = c(1,7)) +
...
<!-- ```{r} -->
<!-- #model  -->
<!-- m2 <- glm(Issue.Food.GMO ~  scale(Spiritual.Identification) * scale(Relid)  + scale(Age) + male , data = dt) -->
<!-- ``` -->
